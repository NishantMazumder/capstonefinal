        ...
        try:
            ...
            for i in y_flow_pred:
                if i == 1:
                    ...
                    self.logger.info("NOTICE 3!! DDoS Attack in Progress!!! {}".format(victim))
                    
                    
                    

            self.logger.info("------------------------------------------------------------------------------")
            legtraf = legitimate_trafic
            total = y_flow_pred
            d = legtraf/len(total)
            d1 = legtraf/len(total)*100

            self.logger.info(legtraf)
            self.logger.info(len(total))
            self.logger.info(d)
            self.logger.info(d1)

            #self.logger.info("legtraf=",legtraf,"len of total = ",len(total))
            #self.logger.info("percentage = ",legtraf/len(total)*100)
            
            #(legitimate_trafic/len(y_flow_pred)*100) > 60:
            if (legitimate_trafic/len(y_flow_pred)*100) > 50:
                self.logger.info("Traffic is Legitimate!")
            else:
                self.logger.info("NOTICE!! DDoS Attack in Progress!!!")
                self.logger.info("NOTICE2 !! DDoS Attack in Progress!!!")
                self.logger.info("NOTICE!! DDoS Attack in Progress!!!")
